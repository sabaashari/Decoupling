...
write.xlsx(country_stat,"../output/country_statistics.xlsx")
...
across_countries <- avg_country_year %>% ungroup() %>% group_by(YEAR_WAVE) %>% 
  summarise(cor_sol_1 = cor(mean_solidarity_1, GDP, use = "complete.obs"),
            ...
            cor_sol_growth = cor(mean_solidarity_1, growth, use = "complete.obs"), 
            cor_agency_growth = cor(agency_score_1, growth, use = "complete.obs"), 
            D_S = 1- cor_sol_1, 
            D_A = 1- cor_agency_1, 
            D = (D_S + D_A)/2, 
...
for(i in 2006:2023){
  ...
  model <- lm(GDP ~ mean_solidarity_1+agency_score_1, data = dd)
  print(summary(model))
  across_countries[across_countries$YEAR_WAVE==i, "R2"] <- 1- summary(model)$adj.r.squared
}
...
write.xlsx(across_countries, "output/across_countries_decoupling.xlsx")


### Time series for Decoupling across counties ####
corr_long <- across_countries %>% pivot_longer(cols = c("R2", "D"), 
...
### Time series for Decoupling across counties for Developed vs Developing ####
corr_long <- across_countries %>% pivot_longer(cols = c("D_S", "D_A", "D", "D_S_growth", "D_A_growth"), 
                                                    names_to = "variable", values_to = "value")
